        ...
        try {
            ...
            if (this._.isObject(filter)) { 
                ...
                if (!keys.includes('state') && !keys.includes('industry')) {
                    throw new Error('Valid object keys for filter should either be state or industry')
                }
                const key = keys[0]
            ...
            if (this._.isString(filter)) {
                const validChoices = ['youngest', 'oldest', 'women', 'female', 'men', 'male', 'real-time']
                if (!validChoices.includes(filter.trim().toLowerCase())) {
                    throw new Error(`Invalid request for filter parameter '${filter}'. Try any of the following valid parameters ${validChoices}`)
                }
